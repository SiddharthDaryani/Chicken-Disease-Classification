...
for filepath in list_of_files:
    ...
    filedir,filename= os.path.split(filepath) # Seperating file directory and filename

    if filedir !='':
        os.makedirs(filedir, exist_ok= True)
        logging.info(f"Creating file directory : {filedir} for the file : {filename}")

    if (not os.path.exists(filepath)) or (os.path.getsize(filepath)== 0): # Logic for creating empty files
